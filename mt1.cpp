// первый рабочий вариант с многопоточностью
...
void worker(char* buf, int start_num, int* buflen) {
    //data.buf.assign(THREAD_BUF_SIZE, 0);
    char* start_buf = buf;
    for (int i = start_num; i < start_num + NUM_PER_THREAD; i+=15) {
        process_chunk(buf, i);
    }
...
void init_and_start_thread_pool(void) {
    for (int i = 0; i < THREAD_NUM and first_non_processed_num + NUM_PER_THREAD < LIMIT; i++) {
        ...
        thread_pool[i].t = std::thread(worker, thread_pool[i].buf.data(), first_non_processed_num, &(thread_pool[i].buflen));
